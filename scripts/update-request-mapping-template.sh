...
for RESOLVER_NAME in $RESOLVER_NAMES; do
  if aws appsync update-resolver --api-id "$API_ID" --type-name "$RESOLVER_TYPE_NAME" --field-name "$RESOLVER_NAME" --request-mapping-template file://"$REQUEST_TEMPLATE_FILE" --response-mapping-template "file://$RESPONSE_TEMPLATE_FILE"; then
